...
Use this reference for Go implementation work when the task touches public APIs, architecture, concurrency, unsafe/cgo code, security, dependencies, testing strategy, or performance claims.
...
- Treat public APIs, build tags, module paths, serialized formats, config files, and CLI behavior as compatibility surfaces.
...
- Prefer named types for identifiers, units, and validated values when plain primitives are ambiguous.
- Keep constructors and parsers responsible for validation so invalid states are hard to create.
- Document invariants on public types and unsafe/cgo-adjacent abstractions.
...
- For FFI, keep Go-side ownership and error translation explicit.

## Concurrency

- Use `context.Context` for cancellation and deadlines across I/O, process, network, and request boundaries.
...
- Avoid goroutine leaks by defining who closes channels and how workers exit.
- Run `go test -race` for touched concurrency code when practical.

## Lint Policy

- Follow the repository's configured `golangci-lint`, `go vet`, and lint profile; do not replace it with a generic command when a project-specific command exists.
- Treat configured linters and CI lint settings as part of the quality gate when the repository enables them.
- Fix lint causes before adding suppressions.
- If a lint suppression is genuinely appropriate, use the narrowest scope and include a short reason.
- Do not add broad package-level suppressions, workspace-level suppressions, or global lint relaxations without explicit approval.
- Keep tests, examples, and benches lint-clean when they are included in the repository's configured lint targets.
- If a lint failure remains, do not call the work complete; report the exact lint, why it remains, and what decision is needed.

## Testing Strategy

- Start with nearby test patterns and fixtures.
- Test public behavior rather than private implementation details.
- Unit-test pure logic and use integration tests for package boundaries, CLI behavior, I/O, or service behavior.
- Use examples for public APIs when they add confidence and remain maintainable.
...
- Before adding a module, check maintenance, license compatibility, security posture, transitive dependency impact, and whether the project already has an approved alternative.
...
- Record command, dataset, machine context, and relevant build tags for performance claims.
...
- Use workspace-level commands when the change crosses package or module boundaries.
- Keep `go.mod`, `go.sum`, workspace files, and build scripts aligned with the repository's dependency policy.

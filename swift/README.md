...
`just check` runs `lint-format`, `lint`, and `build` (the pre-push gate). `build` and `test` **dispatch on project type**: a SwiftPM package (`Package.swift` present) uses `swift build`/`swift test`; an Xcode project (`.xcodeproj`/`.xcworkspace`) uses `xcodebuild`. The Xcode scheme is **resolved automatically** — no placeholder to edit and no XcodeGen assumption: the `_scheme` recipe honors a `$SCHEME` env override, else an XcodeGen `project.yml` `name:`, else the first scheme from `xcodebuild -list`. Override for one invocation with `SCHEME=<name> just build`. The **build/test destination is likewise resolved automatically and is not iOS-specific** — an Xcode scheme may target macOS, iOS, tvOS, watchOS, or visionOS, so the `_destination` recipe asks `xcodebuild -showdestinations` which destinations are eligible for *this* scheme and picks a local one (a Simulator if any, otherwise this Mac), skipping physical devices that would need code signing. Override it with `DESTINATION='platform=macOS' just test` (or any `xcodebuild` destination string). If **neither** a `Package.swift` nor an Xcode project exists yet (e.g. Raven was installed before the app was generated), `build`/`test` **skip cleanly** so your first commit and push are not blocked. `test` is intentionally left out of `check` because an app's UI-test suite is too heavy for every push; run it on demand or in CI, or add `test` back to `check` for a SwiftPM package. Both lint tools scope to **git-tracked Swift only**, so build output (`DerivedData`, `.build`) and vendored SwiftPM checkouts are never formatted or linted. The post-edit hook (`raven-post-edit-format.py`) auto-formats edited `.swift` files with swift-format when available.

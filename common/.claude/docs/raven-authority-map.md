...
| `.claude/skills`                                            | Compatibility                               | Claude Code's skill path, installed as a per-file copy of `.agents/skills`, not a symlink (#274) — a symlink does not survive a checkout without symlink support, the same reason `CLAUDE.md` ships as a plain `@AGENTS.md` import (#253). Do not treat it as a separate source: `.agents/skills` is canonical and the one to edit, and `raven upgrade` refreshes the copy from it. Nothing on the filesystem joins the two, so the same skill exists at both paths and editing the `.claude/skills` copy does not change `.agents/skills`. |

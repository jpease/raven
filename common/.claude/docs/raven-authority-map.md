...
| `.claude/skills`                                            | Compatibility                               | Claude Code compatibility symlink to `.agents/skills`. Do not treat it as a separate source. Exception: if `.claude/skills` already existed as a real directory (not a symlink) before install/upgrade, Raven preserves it as an independent copy instead of converting it — in that case treat it as its own source, since content can exist there with no `.agents/skills` counterpart and edits do not propagate without a re-run of install/upgrade. |

...
Run: `cd /Users/<name>/Developer/github/<downstream-repo> && raven doctor 2>&1 | rg -i "hook manager|husky"`
Expected: an INFO line reporting `hook manager detected (husky)` with the wiring guidance. (Confirms Task 2 end-to-end; the downstream repo uses `core.hooksPath=.husky/_`.)

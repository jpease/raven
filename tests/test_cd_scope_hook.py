...
class EndToEndTests(RavenTestCase):
    ...
    def test_gemini_payload_receives_gemini_hook_event_and_system_message(self):
        payload = {
            ...
            "tool_input": {"command": "cd /srv/app && git status"},
            "session_id": "gemini-s-1",
        }
        env = dict(os.environ, TMPDIR=str(self.destination))
        result = subprocess.run(
            [sys.executable, str(HOOK)],
        ...
        self.assertIn("git -C /srv/app status", data["systemMessage"])
        self.assertEqual(data["hookSpecificOutput"]["hookEventName"], "BeforeTool")
        self.assertIn("git -C /srv/app status", data["hookSpecificOutput"]["additionalContext"])

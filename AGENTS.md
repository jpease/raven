...
- A line the checker flags but that is legitimate (this rule's own prose, a test fixture asserting the detector works, documentation *about* the pattern) can carry a trailing `raven-hygiene: allow` comment to suppress just that line — visible in the diff, reviewable, and not a file- or directory-level exclusion. A path-level finding has no line to carry that marker; rename the path instead. Keep the local name list itself out of tracked files: in local memory or `.git/info/`-excluded config; putting it in a tracked file recreates the leak.
...
- Config comparison parses the committed and staged blobs of the same file rather than reading the diff, so a relaxation buried in a reformat is still seen. Only `pyproject.toml`, `ruff.toml`, `pyrightconfig.json`, `mypy.ini`, and `setup.cfg` are compared; `raven_lib/gate_config.py` records why ESLint's and RuboCop's formats are not.
- The `raven-hygiene: allow` marker suppresses a relaxation finding on the line it appears on, the same as a hygiene finding. For a config file the marked line is dropped before parsing, so it never enters the comparison. A test-count finding has no line of its own — a marker on any added line in that file suppresses it.
- `raven assess` reports the same relaxation as a standing state rather than a delta: a ruff config that selects a rule family and then ignores all of it, a `per-file-ignores` keyed on `*`, an `exclude` covering the whole tree, or a `typeCheckingMode` below the `standard` floor the python template ships. That half is shipped to installed projects; the staged check above is repo-only, like `check-staged-hygiene.py`.

...
A nested payload must be **sliced out of the original tokens**, never rebuilt from normalized positionals. Rebuilding drops the payload's own flags, so `xargs rm -rf /` reduced to `rm /` and every flag-keyed rule missed it one layer down while the positional-keyed ones (`xargs -I{} kubectl delete pod {}`) still matched — the same asymmetry as #209, reappearing inside the function that fixed it (issue #214). `ssh` is the one branch that may rebuild, and only because its remote command conventionally arrives as a single quoted token; the unquoted spelling has the identical defect. Any new branch whose payload is unquoted and multi-token belongs on the `_first_operand_index` path.

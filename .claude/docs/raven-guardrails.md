...
- `.claude/hooks/raven-pre-edit-guard.py` blocks edits to protected secret-like files, warns on high-churn paths, and — for paths a project lists under `[edit_guard] protected_paths` in `.raven/config.toml` (#247) — escalates the edit to a permission prompt on Claude Code, or adds a warning to context on Codex and Gemini CLI. This is the mechanical backstop for the Pause-And-Ask categories a project can spell as paths; the judgment-call categories stay prose.
...
- `.claude/hooks/raven-pre-bash-cd-scope.py` adds one advisory sentence, once per session, when a command opens with `cd <dir> &&` and the command after it is read-only and already takes a path operand — `git` (rewritten as `git -C <dir>`) and the search/read tools. `just`, package managers, and anything resolving config from the working directory are left alone, because there the `cd` is correct. A leading `cd` puts a command's meaning in state the command does not carry, and the harness resets the working directory between calls; Claude Code 2.1.259 also escalates the shape as a directory it cannot determine (#260), but the hook's reason does not depend on that behavior.
- `.claude/hooks/raven-post-bash-summarize.py` nudges noisy commands toward RTK when exact raw output is not required.
- `.claude/hooks/raven-post-bash-truncate.py` (Claude Code and Gemini CLI) replaces a Bash result longer than `[bash_output] max_lines` with its head, its tail, and the path of a file holding all of it. A tool result is re-read on every later turn, so this is where the token pillar is enforced rather than advised; Codex's PostToolUse hook cannot replace a result, so that host relies on its own `tool_output_token_limit`.
...
| Hook (`raven-pre-bash-guard.py`, `raven-pre-edit-guard.py`) | A Python process Claude Code, Codex, and Gemini CLI all spawn per tool call | Tokenizes the command: normalizes option clusters, resolves wrappers, follows nested payloads | Every host |
...
These rules no longer live in `permissions.deny`. Claude Code 2.1.259 began evaluating a Bash command's implied read scope against them at *directory* granularity, so every recursive search of the repository root escalated because some `.pem` could exist inside it -- no matching file had to be present. That put a prompt on routine discovery and trained approval reflex, which is the opposite of what a secrets guard is for (#260). A settings-only fix does not exist: rules evaluate deny -> ask -> allow, so no allow entry can carry an exception, and anchoring the globs so they stop matching at depth would drop `packages/api/.env` from coverage. `.claude/hooks/raven-pre-read-secret-guard.py` enforces the same 21 rules against the concrete path a `Read` names, and `[hooks] block_secret_reads = false` turns it off. The depth semantics recorded below are why the hook matches basenames and directory segments rather than whole paths.
